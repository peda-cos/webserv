...
## Por que o Projeto exige poll() (ou equivalente)
- O Projeto proíbe `fork()` para clientes — logo, sem threads ou processos extras.
...
- O Projeto permite qualquer um deles — escolha `poll()` (Linux/Mac) ou `epoll()` (só Linux).
...
- **NUNCA fazer `recv()` ou `send()` sem passar por `poll()` antes** — regra explícita do Projeto.
