...
- **A Temível Barra Final (Trailing Slash):**
  ...
  Qual é a fundamentação disto no Projeto? Se o browser receber do Autoindex de links sem barra a renderização procedural do servidor e o root renderizar na tabulação de `a href` um clique pro ficheiro `desenho.png`, no HTML real de client-side ele enviaria uma repetição de path quebrada indo bater diretamente no `/desenho.png` (raiz fictícia). Forçando nativamente a barra o cliente pede tecnicamente ao Webserv o URI estrito: `"GET /imagens/desenho.png HTTP/1.1"`.
